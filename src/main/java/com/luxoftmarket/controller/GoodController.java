...
public class GoodController {
 ...
 https://www.youtube.com/watch?v=9GdtWiovvIQ хорошо объ€снено про инкапсул€цию
...
//        model.addAttribute("message", "new my massage");
    ...
    public String addGood(Model model) {                        //возвращает страницу addBook.jsp в которой будет форма дл€ добавлени€ новой книги с кнопкой add book
//            model.addAttribute("book", new Book());
    ...
    @PreAuthorize("isAuthenticated()")
    public String addGood(@ModelAttribute("good") User user, BindingResult bindingResult) { //будет добавл€ть новую книгу в базу данных, добавили Binding Result который будет хранить в себе ошибки
//            this.bookValidator.validate(book, bindingResult);                                   //валидации
//            if (bindingResult.hasErrors()) { //перед сохранением книги в базу, провер€ем, с помощью созданного валидатора, нашу модель на предмет ошибок ? и все ошибки записываютс€ в Binding Result
        return "addGood"; // если есть ошибки, возвращаем ту-же самую вьюху
    }
//        System.out.println("∆анр: " + book.getGenre() + ", книга " + book.getName());
